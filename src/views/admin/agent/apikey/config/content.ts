...
import { AgentApikeyAPI } from "@/api";
...
const contentConfig: IContentConfig = {
  ...
  indexAction: (query) => AgentApikeyAPI.getApiKeyList(query),
  parseData: (res) => ({ total: res.data.total, list: res.data.list || [] }),
  modifyAction: (row, field, value) => {
    return AgentApikeyAPI.updateApiKey({ id: row.id, [field]: value });
  },
  deleteAction: (ids) => {
    const idList = ids.split(",");
    if (idList.length === 1) {
      return AgentApikeyAPI.deleteApiKey({ id: parseInt(idList[0]) });
    }
    return Promise.all(idList.map((id) => AgentApikeyAPI.deleteApiKey({ id: parseInt(id) }))).then(
      () => ({
        code: 0,
        message: "success",
      })
