...
const contentConfig: IContentConfig = {
  ...
  indexAction: (query) => AgentEngineAPI.getEngineList(query),
  parseData: (res) => ({ total: res.data.total, list: res.data.list || [] }),
  modifyAction: (row, field, value) => {
    switch (field) {
      case "is_default":
        return AgentEngineAPI.setDefaultEngine({ id: row.id, is_default: row.is_default });
    }

    return AgentEngineAPI.updateEngine(row as UpdateEngineReq);
  },
  deleteAction: (ids) => {
    const idList = ids.split(",");
    if (idList.length === 1) {
      return AgentEngineAPI.deleteEngine({ id: parseInt(idList[0]) });
    }
    return Promise.all(idList.map((id) => AgentEngineAPI.deleteEngine({ id: parseInt(id) }))).then(
      () => ({
        code: 0,
        message: "success",
      })
