...
export const PaymentAccountAPI = {
  /** 获取用户账户列表 */
  getUserAccountList(params?: GetUserAccountListReq): Promise<IApiResponse<GetUserAccountListResp>> {
    return request({
  ...
  adjustUserBalance(data?: AdjustUserBalanceReq): Promise<IApiResponse<AdjustUserBalanceResp>> {
    return request({
      url: `/api/v1/payment/accounts/adjust`,
      method: "POST",
      data: data,
    });
  },
  ...
  /** 更新用户信用额度 */
  adjustUserCreditLimit(data?: AdjustUserCreditLimitReq): Promise<IApiResponse<AdjustUserCreditLimitResp>> {
    return request({
      url: `/api/v1/payment/accounts/credit-limit`,
      method: "POST",
      data: data,
    });
  },
